//------------------------------------------------------------------------------
// <auto-generated>
//     這段程式碼是由工具產生的。
//
//     變更這個檔案可能會導致不正確的行為，而且如果已重新產生
//     程式碼，這個檔案將會遺失。
// </auto-generated>
//------------------------------------------------------------------------------

using System;
using System.Collections.Generic;
using System.Data;
using System.Runtime.Serialization;
using System.Text;

using Ares.Cimes.IntelliService.DbTableSchema;

namespace Ares.Cimes.IntelliService.DbTableRow
{
    /// <summary>
    /// CST_API_COOLING_DOWN_TEMP_Row, 
    /// </summary>
    [Serializable]
    public class CST_API_COOLING_DOWN_TEMP_Row : DBTableRowBase
    {            
        /// <summary>
        /// 取得該 Row 是屬於 CST_API_COOLING_DOWN_TEMP Table, 傳回代表該 Table Schema 的唯一實體
        /// </summary>
        public override DBTableSchemaBase TableSchema
        {
            get { return CST_API_COOLING_DOWN_TEMP.Singleton; }
        }
    
        #region Ctor()
        /// <summary>
        /// 建構一個空的 CST_API_COOLING_DOWN_TEMP_Row 物件.
        /// 此 Ctor 僅供 Cimes 內部 Generator 使用, 若要取得一 Row 實體, 請使用 Schema.NewRow()
        /// </summary>
        public CST_API_COOLING_DOWN_TEMP_Row() : base() {  }

        /// <summary>
        /// 以原生 DataRow 建構一個 CST_API_COOLING_DOWN_TEMP_Row 物件
        /// </summary>
        /// <param name="row">原生 DataRow</param>
        public CST_API_COOLING_DOWN_TEMP_Row(DataRow row) : base(row) {  }
        
        /// <summary>
        /// For Serializable Operation
        /// </summary>
        /// <param name="si">SerializationInfo</param>
        /// <param name="context">StreamingContext</param>
        protected CST_API_COOLING_DOWN_TEMP_Row(SerializationInfo si, StreamingContext context) : base(si, context) {  }
        #endregion

                    /// <summary>
    /// 降溫溫度記錄表LOG SID 資料型態:NVARCHAR2(100) 是否允許Null:Y 
    /// </summary>
    public String API_COOLING_DOWN_TEMP_SID { 
      get { return (this["API_COOLING_DOWN_TEMP_SID"] is DBNull) ? string.Empty : (String) this["API_COOLING_DOWN_TEMP_SID"]; } 
      set { this["API_COOLING_DOWN_TEMP_SID"] = string.IsNullOrEmpty(value) ? DBNull.Value : (object) value; } 
    } 

    /// <summary>
    /// 工件編號 資料型態:NVARCHAR2(100) 是否允許Null:Y 
    /// </summary>
    public String COMPLOT { 
      get { return (this["COMPLOT"] is DBNull) ? string.Empty : (String) this["COMPLOT"]; } 
      set { this["COMPLOT"] = string.IsNullOrEmpty(value) ? DBNull.Value : (object) value; } 
    } 

    /// <summary>
    /// 溫度 資料型態:NVARCHAR2(100) 是否允許Null:Y 
    /// </summary>
    public String TEPM { 
      get { return (this["TEPM"] is DBNull) ? string.Empty : (String) this["TEPM"]; } 
      set { this["TEPM"] = string.IsNullOrEmpty(value) ? DBNull.Value : (object) value; } 
    } 

    /// <summary>
    /// WIP歷史記錄LINKSID 資料型態:NVARCHAR2(100) 是否允許Null:Y 
    /// </summary>
    public String LINKSID { 
      get { return (this["LINKSID"] is DBNull) ? string.Empty : (String) this["LINKSID"]; } 
      set { this["LINKSID"] = string.IsNullOrEmpty(value) ? DBNull.Value : (object) value; } 
    } 

    /// <summary>
    /// 使用者 資料型態:NVARCHAR2(100) 是否允許Null:Y 
    /// </summary>
    public String USERID { 
      get { return (this["USERID"] is DBNull) ? string.Empty : (String) this["USERID"]; } 
      set { this["USERID"] = string.IsNullOrEmpty(value) ? DBNull.Value : (object) value; } 
    } 

    /// <summary>
    /// 更新時間 資料型態:NVARCHAR2(100) 是否允許Null:Y 
    /// </summary>
    public String UPDATETIME { 
      get { return (this["UPDATETIME"] is DBNull) ? string.Empty : (String) this["UPDATETIME"]; } 
      set { this["UPDATETIME"] = string.IsNullOrEmpty(value) ? DBNull.Value : (object) value; } 
    } 


        
        /// <summary>
        /// 取得該 Row 的深層複製實體
        /// </summary>              
        /// <returns>深層複製實體</returns>
        public new CST_API_COOLING_DOWN_TEMP_Row DeepCopy()
        {
            return (CST_API_COOLING_DOWN_TEMP_Row) base.DeepCopy();
        }
    }
}


